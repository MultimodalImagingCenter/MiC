...
public class MaskIInstantComparator3D implements PlugIn {
    ...
    public boolean analysis(){
        int maxTruth = correctObjectNumbering(truthMaskIP);
        int maxTest = correctObjectNumbering(testMaskIP);
        ...
        ImageProcessor histo2D = histo2D(truthMaskIP,maxTruth,testMaskIP,maxTest);
        //pixel analysis
        ...
        if(objectMethod || pixelObjectMethod) {
            ...
            int[] histoTruth=histo1D(truthMaskIP,maxTruth);
            int[] histoTest=histo1D(testMaskIP,maxTest);
            //compute IoUs for each objects
            ImageProcessor iou = computesIoUs(histo2D, histoTruth, histoTest);
...
//        If wanted, show image (with display range adapted to range of intensity values present)
    ...
    private int correctObjectNumbering(ImagePlus imp){
        StackStatistics sstats=new StackStatistics(imp);
        int[] histo= histo1D(imp, (int)Math.round(sstats.max));
        int max=-1;
    ...
    private int correctObjectNumbering(ImageProcessor ip, int[] convert){
        int max=-1;
    ...
    private int[] conversionIndexes(int[] histo){
    ...
    private int[] histo1D(ImagePlus imp, int max){
    ...
    private int[] histo1D(ImageProcessor ip, int[] histo){
        for(int y=0;y<ip.getHeight();y++){
            for(int x=0;x<ip.getWidth();x++){
    ...
    private ImageProcessor histo2D(ImagePlus imp1, int max1, ImagePlus imp2, int max2){
    ...
    private ImageProcessor histo2D(ImageProcessor ip1, ImageProcessor ip2, ImageProcessor histo){
    ...
    private ImageProcessor computesIoUs(ImageProcessor histo2D, int[] histoTruth, int[] histoTest){
